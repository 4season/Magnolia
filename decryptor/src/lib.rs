pub mod decryptor {
    ...
    const PBE_PASSWORD: &[u8; 34] = &[
        0, 22, 0, 8, 0, 9, 0, 111, 0, 2, 0, 23, 0, 43, 0, 8, 0, 33, 0, 33, 0, 10, 0, 16, 0, 3, 0,
        3, 0, 7, 0, 6, 0, 0,
    ];
    ...
    #[unsafe(no_mangle)]
    pub extern "C" fn decrypt_data_ffi(
        user_id: c_ulonglong,
        i9: usize,
        ciphertext_b64_ptr: *const c_char,
    ) -> *mut c_char {
        let ciphertext_b64 = unsafe {
            if ciphertext_b64_ptr.is_null() {
                eprintln!("[Rust FFI Error]: ciphertext_b64_ptr가 null입니다.");
                return ptr::null_mut();
            }
            CStr::from_ptr(ciphertext_b64_ptr).to_str().unwrap_or("")
        };
        match decrypt_data(user_id, i9, ciphertext_b64) {
            Ok(decrypted_string) => CString::new(decrypted_string).unwrap().into_raw(),
            Err(error) => {
                eprintln!("[Rust FFI Error]: {}", error);
                ptr::null_mut()
            }
        }
    }
    ...
    pub extern "C" fn free_string(ptr: *mut c_char) {
        if ptr.is_null() {
            eprintln!("[Rust FFI Error]: null pointer");
            return;
        }
        unsafe {
